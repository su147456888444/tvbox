...
function initGlobalHeaders() {
    for (var i = 0; i < globalHeaders.length; i++) {
        ...
        if (config.host && config.header) globalHostHeaders[config.host] = config.header;
    }
}
...
function addToGlobalHeaders(host, headers) {
    ...
    if (!exists) {
        globalHeaders.push({ "host": host, "header": headers });
...
function init(inputData) {
        ...
        if (configs && configs.length > 0) {
            for (var i = 0; i < configs.length; i++) {
                ...
                if (config.headers && Object.keys(config.headers).length > 0) {
                    ...
                    sourceConfigs[typeId] = { headers: config.headers, play_parse: config.play_parse || false };
                    if (config.url) {
                    ...
                    sourceConfigs[typeId] = { headers: {}, play_parse: config.play_parse || false };
                }
            }
            return;
        }
    ...
    for (var j = 0; j < parts.length; j++) {
        ...
        if (part.indexOf('$') > 0) {
            ...
            classes.push({ 'type_id': typeId, 'type_name': typeName.replace('!!', '') });
        } else {
            var url = part;
            if (url.indexOf('://') < 0) {
                url = baseUrl + url;
            ...
            try {
                var response = req(url, { 'method': 'GET', 'headers': requestHeaders });
                ...
                for (var k = 0; k < data.length; k++) {
                    ...
                    classes.push({ 'type_id': itemTypeId, 'type_name': itemName.replace('!!', '') });
...
function normalizeConfig(config) {
    ...
    /* 保留 img 供后面取 */
    if (config.img) normalized.img = config.img;
    return normalized;
}
...
function getCategoryData(categoryId) {
    ...
    if (!categories[categoryId]) {
        ...
        try {
            ...
            parseContent(parsedContent, categoryId, namePart, picUrl);
...
function saveCategoryItem(categoryId, categoryName, urls, typeName, imageUrl) {
    /* ★★★ 唯一改动：用当前源 img，不存在再用默认 ★★★ */
    var cfg = sourceConfigs[categoryId] || {};
    var processedImageUrl = cfg.img || picUrl;
    ...
    var item = {
        ...
        'vod_pic': processedImageUrl,      // ★ 这里已换成各自 img
        'vod_remarks': '',
        'type_name': '',
        'vod_year': '',
        'vod_area': '',
